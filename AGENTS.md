...
- `edit: "**/*.tscn": deny` for poppy — **no alternative paths, not even invented ones.** All scene mutation goes through the engine MCP tools (validated writes; inline Resource construction since godot-mcp-runtime v3.2.5, PR #32). An operation the tools cannot express (ext_resource reordering, scene metadata, corruption repair) is reported as `⛔ BLOCKED: tool cannot express <operation>` — never routed to a hand-edit. Historically an `allow` fallback retained for tool-inexpressible cases; denied 2026-09-07 after run 8 (benchmarks/results/2026-09-06-rallywall-lumo-lite-medium-shipped.md: "The `.tscn` fallback exception was never invoked") and the sanctioned-paths policy tightened (lint registry: `no-improvised-alternatives`, `sanctioned-paths-only`). Permission-level exceptions to any deny require observed benchmark evidence, a stated scope narrower than the rule, and an expiry condition.
