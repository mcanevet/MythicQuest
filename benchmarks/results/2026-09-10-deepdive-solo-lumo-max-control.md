...
Both arms SHIPPED a playable, verified DeepDive. Swarm's shipped with a rework cycle (dead pearls, HUD overflow, hollow loop found by pootie, fixed, re-verified); solo's shipped first-pass with a self-found-and-fixed `look_at` bug and candid self-critique ("steep early difficulty", "critique-lite only"). Neither arm detected the dominant-strategy bait as a vision-level issue (swarm caught adjacent issues but never issued REVISE_VISION; solo didn't engage). On the objective triad: **Outcome** — near-parity, swarm marginally ahead on adversarial polish (consumer-critique round); **Speed** — solo 45% of swarm wall-clock; **Tokens** — solo 38% cache-adjusted / 51% raw. Solo wins speed+tokens decisively; swarm wins outcome assurance, but by less than the architecture implies, because solo's gauntlet discipline substituted for rachel's independence surprisingly well.
...
The solo arm is viable and dramatically cheaper/faster — but its marginal risks are exactly where predicted: adversarial critique depth and voluntary-stop reliability, not implementation correctness. The multi-agent architecture's demonstrable value is concentrated in the critic loop, which the token budget says costs ~2.6× — arguably worth it for consumer-grade games, defensibly optional for prototype-grade ones.
