...
5. **Upstream (godot-mcp-runtime):** (a) classify "engine unresponsive (possibly OS-suspended under host memory pressure)" distinctly from "game not running" — the current message actively misled a capable agent for hours; (b) a health pre-check distinguishing socket-bound-but-not-servicing from connection-refused would have made the suspension diagnosis mechanical.
