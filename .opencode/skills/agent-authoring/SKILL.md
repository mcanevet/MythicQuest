...
### How opencode evaluates permissions (verified from source, Sep 2026)

Read this before touching any frontmatter — mis-keyed rules are silently
inert, not errors. Sources: `packages/opencode/src/permission/index.ts`,
`packages/core/src/permission.ts`, `packages/core/src/tool/{write,edit,bash,read}.ts`,
`packages/opencode/src/tool/{task,skill}.ts`, `packages/opencode/src/mcp/catalog.ts`
in the anomalyco/opencode repo.

**Keys (the "action"/"permission" side of a rule).** A rule only ever matches
against these exact keys — any other key (`write:`, `mcp:`, ...) is dead config
that opencode ignores without warning:

- `edit` — governs ALL file modifications: the `edit`, `write`, AND
  `apply_patch` tools all assert `action: "edit"` (write.ts:80). **There is no
  separate `write` permission key** — declaring `write:` in frontmatter does
  nothing. Scope by resource path (`"reports/**": allow`) on the `edit` key.
- `read` — read tool; also MCP resource reads, which use patterns of the form
  `mcp:<server>:<uri>` against the `read` key (not a tool-name match).
- `bash` — shell; pattern matched against the full command string (parsed
...
- **MCP tools** — the permission key is the FULL TOOL NAME,
  ...
  key. A hypothetical `"godot-mcp-runtime"` (bare server name) matches
  nothing — always keep the `_` and trailing tool wildcard.

**Evaluation (both V1 and V2):** `findLast` over the flattened ruleset —
**last matching rule wins**, so put catch-alls (`"*": deny`) FIRST and
narrower grants after. Unmatched → `"ask"` (interactive approval); in
headless/benchmark runs an "ask" cannot be answered, so effectively treat
unlisted actions as unavailable. Agent frontmatter rules are merged with
global/project config (agent rules last → they win conflicts).

**Deny semantics:** a matching `deny` is checked first and blocks outright
(it never degrades to "ask"). Additionally, `deny` with action `*` and
resource `*` makes opencode **hide the tool from the model entirely**. Note
MCP tool keys are also subject to this: a broad `"*: deny` typo'd to match a
server-prefixed tool name will hide the engine toolset.

**Subagent inheritance gotcha:** opencode auto-appends `task: "*": deny` and
`todowrite: "*": deny` to subagent sessions that don't declare those keys —
subagents can't spawn subagents or write todos unless explicitly granted.

**Mistake this section exists to prevent:** we twice wrote `write:` keys in
frontmatter believing opencode distinguished write from edit. It does not —
verify against source, not docs summaries, when a permission seems
inert.
