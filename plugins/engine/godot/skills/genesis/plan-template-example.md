# Godot worked example of the genesis task-plan template

Concrete Godot scaffold filling in `skills/genesis/reference/plan-template.md`
(generic). Use when the engine plugin is godot.
...
Invariant rule names must come from `./.opencode/plugins/engine/godot/skills/setup-project/reference/testing-patterns.md` — an unknown `rule` value is silently ignored (no-op invariant).
...
5. **RigidBody2D critical config** (if entity uses RigidBody2D):
   ...
   - Full pattern in `./.opencode/plugins/engine/godot/skills/create-scene-with-script/reference/physics-nodes.md`
...
7. **Testability**: Declare invariants using the engine-agnostic schema in `./.opencode/plugins/engine/godot/skills/setup-project/reference/testing-patterns.md` (bot types, invariant rules, metrics)
8. **Verification**:
