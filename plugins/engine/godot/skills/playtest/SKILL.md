...
The framework uses genre-agnostic bots (chaos, pursuit, replay, nav_agent) and invariants — see `./.opencode/plugins/engine/godot/skills/setup-project/reference/testing-patterns.md` for the configuration schema.
...
4. **Generate formatted report:** pipe the JSON report file through `./.opencode/plugins/engine/godot/skills/playtest/scripts/render_report.py <report.json>` (exit 1 if violations present).
