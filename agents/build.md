...
**Task batching (token economy):** each subagent session pays a fixed ~250k-token context boot (agent prompt, MCP tool descriptions, skill files) before doing any work. Batching 2-3 tightly-related tasks into ONE delegation amortizes that boot cost across them — observed 09-03: ~550k input tokens per single-task delegation, dominated by the boot. Batch when the tasks form one coherent unit (same entity/system: e.g. "ball physics + paddle + score on catch"), share files, or one is trivial scaffolding for the other. Do NOT batch unrelated features just to save a delegation — a failed batch wastes its whole budget, so keep batches small (2-3 max) and cohesive. For a batched delegation: pass briefs for all tasks (see below), have backlog-grooming mark each `[in progress]` with its own plan file, and require log-result per task.
